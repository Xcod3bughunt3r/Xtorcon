...
Example:
    ./tor_httpd.py -p 8080 -d /opt/files/
'''
...
def main():
    ...
    reactor.addSystemEventTrigger(
        'before', 'shutdown',
        functools.partial(xtorcon.util.delete_file_or_tree, hs_temp)
    )

    # Add the hidden service to a blank configuration
    config = xtorcon.TorConfig()
    config.SOCKSPort = 0
    config.ORPort = 9089
    config.HiddenServices = [
        xtorcon.HiddenService(
            config, hs_temp,
    ...
    tordeferred = xtorcon.launch_tor(config, reactor,
